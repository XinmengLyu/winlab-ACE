...
class Camera(object):
	...
	def __init__(self, debug=False, bus_number=1, db="config"):
		...
		self._pan_offset = int(self.db.get('pan_offset', default_value=0))
		self._tilt_offset = int(self.db.get('tilt_offset', default_value=0))
	...
	def pan_offset(self,value):
		if not isinstance(value, int):
			raise TypeError('"pan_offset" must be "int"')
		self._pan_offset = value
		self.db.set('pan_offset', value)
		self.pan_servo.offset = value
		self.pan_servo.write(self.CALI_PAN)
	...
	def tilt_offset(self, value):
		if not isinstance(value, int):
			raise TypeError('"tilt_offset" must be "int"')
		self._tilt_offset = value
		self.db.set('tilt_offset', value)
		self.tilt_servo.offset = value
		self.tilt_servo.write(self.CALI_PAN)
